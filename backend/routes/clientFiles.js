...
function exam_repeat_filename(cnt,filename,path,callback){
    ...
    db.sql('select 1 from upload_files where filepath = \''+filepath+'\'', 
    function(err, result) {
        if(err) {
    ...
    req.files.forEach(file => {
        ...
        exam_repeat_filename(0,file.originalname,filepath,
            function(filename,filepath){
                db.sql('insert into upload_files (filename,filepath,uploader) values (\''+
                filename+'\',\''+filepath+'\',\''+req.session.username+'\')', function(err, result) {
                    if(err) {
                        console.log("sql错误", err);
...
router.get('/', function(req, res, next) {
    ...
    db.sql('select * from upload_files order by createtime desc;', function(err, result) {
        if(err) {
            console.log("sql错误", err);
...
router.delete('/', function(req, res, next) {
    ...
    db.sql('delete from upload_files where filepath = '+filepath, function(err, result) {
        if(err) {
            console.log("sql错误", err);
            res.send({"status":"fail","message":err.message});
            return;
